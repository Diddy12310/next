<template>
  <div class="paradox">
          ...
          <div v-if="item.live">
            ...
            <v-card-title primary-title>
              <div>
                ...
                <h4 class="text-body-1 grey--text">
    ...
    <v-dialog v-model="dialog" max-width="800" v-if="current">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-spacer></v-spacer>
          <v-btn text color="blue accent-1" @click="dialog = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-btn
      v-if="$root.user.rights.author"
      color="deep-purple"
      fab
      fixed
      bottom
      right
      @click="new_news.open = true"
    >
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field
            hide-details="auto"
            label="Cover URL"
            v-model="new_news.cover"
          ></v-text-field>
          <p class="grey--text font-weight-light" v-if="new_news.cover">
            Does the story's cover display correctly?
          </p>
          <v-img
            max-width="200"
            loading="lazy"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
            :disabled="!new_news.title || !new_news.content || !new_news.cover"
            text
            color="blue accent-1"
            @click="addNews()"
            >Submit</v-btn
...
export default {
  ...
  methods: {
    async getNews() {
      await this.$http
        .get("https://www.theparadigmdev.com/api/paradox/get")
        .then((response) => {
    ...
    addNews() {
      if (this.new_news.title && this.new_news.content && this.new_news.cover) {
        this.$http
          .post("https://www.theparadigmdev.com/api/paradox/add", {
