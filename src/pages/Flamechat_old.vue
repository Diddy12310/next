				...
				<v-card class="chat-card" v-if="ready && chatroom_id">
					<v-card-text>
						<!-- <ul class="messages" v-chat-scroll="{ always: false }">
							...
							<li v-for="message in messages" :key="message.id" :id="message.id">
								<v-layout row wrap>
									<v-flex xs10>
										<v-list-item style="cursor: pointer;" v-ripple @click="openUsernamePopup(message.name)">
						...
						<v-list rounded two-line>
							<v-list-item v-for="(message, index) in messages" :key="index" @click="openUsernamePopup(message.name)">
								<v-list-item-avatar>
									<v-img :src="message.pic"></v-img>
								</v-list-item-avatar>
								<v-list-item-content>
									<v-list-item-title class="text-uppercase font-weight-medium" :style="{ 'color': message.color }" v-html="message.name"></v-list-item-title>
					...
					<v-card-actions>
						<form @submit.prevent="sendChat" class="new-message" v-if="!editor">
							<v-layout row text-center align-center justify-center>
								<v-flex xs11><v-text-field :disabled="!flamechat_enable" class="message-box" autocomplete="off" label="Message..." v-model="newMessage"></v-text-field></v-flex>
						...
						<form @submit.prevent="editChat" class="new-message" v-if="editor">
							<v-layout row text-center align-center justify-center>
								<v-flex xs11><v-text-field :disabled="!flamechat_enable" class="message-box" autocomplete="off" label="Edit message..." v-model="editMessage"></v-text-field></v-flex>
