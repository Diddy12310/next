        ...
        <v-fade-transition group hide-on-leave>
          <v-row key="list" v-if="!$root.profile">
            <v-col
              ...
              md="4"
              cols="12"
              v-for="(user, index) in filtered_people"
              :key="index"
              class="text-center"
            >
              <v-card @click="$root.profile = user" class="pa-4 fill-height">
                ...
                <h1
                  class="mt-6 text-h4 font-weight-medium"
          ...
          <v-row key="profile" v-if="$root.profile">
            <v-col xs="12" md="4" cols="12">
              ...
              <div class="text-center">
                ...
                <h1
                  class="mt-6 text-h4 font-weight-medium"
                  :style="{ color: $root.profile.color }"
                >
                  {{ $root.profile.username }}
                </h1>
                <p class="grey--text">{{ $root.profile.bio }}</p>
                <v-btn v-if="is_sent" block text disabled class="mb-2"
                  ><v-icon left>mdi-account-plus</v-icon>Friend Request
                  Pending...</v-btn
                >
                <v-btn
                  v-else-if="is_approved"
                  @click="removeFriend()"
                  block
                  text
                  class="mb-2"
                  color="blue"
                  ><v-icon left>mdi-account-minus</v-icon>Remove
                  Friend...</v-btn
                >
                <v-btn
                  v-else
                  ...
                  color="blue"
                  class="mb-2"
                  ><v-icon left>mdi-account-plus</v-icon> Add Friend</v-btn
                >
                <v-btn
                  ...
                  color="red"
                  class="mb-2"
                  ><v-icon left>mdi-account-cancel</v-icon>Block</v-btn
                >
                <v-btn v-if="is_blocked" block text color="red" class="mb-2"
                  ><v-icon left>mdi-account-cancel</v-icon>Unblock</v-btn
                >
              </div>
            </v-col>
            ...
            <v-col xs="12" md="8" cols="12">
              ...
              <v-card
                ...
                max-width="400"
                v-for="(post, index) in $root.profile.posts"
                :key="index"
              >
                <v-img
                      ...
                      <v-list-item class="grow">
                        ...
                        <v-list-item-content>
                          <v-list-item-title
                            ...
                            :style="{ color: $root.profile.color }"
                            >{{ $root.profile.username }}</v-list-item-title
                          >
...
export default {
    ...
    filtered_people() {
      ...
      this.people.forEach((person) => {
        ...
        if (
          ...
          !this.$root.user.people.blocked_by.includes(person._id) &&
          !blocked
        )
          people.push(person);
      });
      var filteredPeople = people.filter((person) => {
        return (
          person.username.toLowerCase().includes(this.search.toLowerCase()) ||
          person.bio.toLowerCase().includes(this.search.toLowerCase()) ||
          person.bio.toLowerCase().includes(this.search.toLowerCase())
        );
      });
  ...
  methods: {
    addFriend() {
      this.$http.get(
        `https://www.theparadigmdev.com/api/users/${this.$root.user._id}/people/send/${this.$root.profile._id}`
      );
    },
    removeFriend() {
      this.$http
        .get(
          `https://www.theparadigmdev.com/api/users/${this.$root.user._id}/people/remove/${this.$root.profile._id}`
        )
        .then((response) => {
    ...
    blockPerson() {
      this.$http
        .get(
          `https://www.theparadigmdev.com/api/users/${this.$root.user._id}/people/block/${this.$root.profile._id}`
        )
        .then((response) => {
    ...
    likePost(id, index) {
      this.$http
        .get(
          `https://www.theparadigmdev.com/api/broadcast/${this.$root.user._id}/like/${this.$root.profile._id}/${id}`
        )
        .then((response) => {
          this.$root.profile = response.data.profile;
          this.$root.user = response.data.user;
    ...
    unLikePost(id, index) {
      this.$http
        .get(
          `https://www.theparadigmdev.com/api/broadcast/${this.$root.user._id}/unlike/${this.$root.profile._id}/${id}`
        )
        .then((response) => {
          this.$root.profile = response.data.profile;
          this.$root.user = response.data.user;
