  ...
  <div class="satellite">
    <v-toolbar dense color="blue-grey darken-2">
      ...
      <v-menu offset-y v-model="quick_links">
        <template v-slot:activator="{ on: menu }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on: tooltip }">
              <v-btn text icon v-on="{ ...tooltip, ...menu }"
                ><v-icon>{{
                  quick_links ? "mdi-chevron-up" : "mdi-chevron-down"
                }}</v-icon></v-btn
              >
            </template>
        ...
        <v-list dense>
          <v-list-item
            :input-value="input == 'https://www.bing.com/'"
            @click="input = 'https://www.bing.com/'"
          >
            <v-list-item-icon><v-icon>mdi-magnify</v-icon></v-list-item-icon>
            <v-list-item-title>Search</v-list-item-title>
          </v-list-item>
          <v-list-item
            :input-value="input == 'https://www.theepochtimes.com/'"
            @click="input = 'https://www.theepochtimes.com/'"
          >
