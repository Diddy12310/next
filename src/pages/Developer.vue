            ...
            <template v-slot:default>
              ...
              <tbody>
                <tr
                  ...
                  :key="index"
                  :style="{
                    'text-decoration': bug.fixed ? 'line-through' : 'none',
                  }"
                  :class="{ 'grey--text text--darken-1': bug.fixed }"
                >
                  <td>{{ bug.title }}</td>
                  <td>{{ bug.category }}</td>
                  <td>{{ bug.location }}</td>
                  ...
                  <td v-if="!bug.fixed">
                    <v-icon
                      ...
                      @click="fixBug(bug._id)"
                      >mdi-check</v-icon
                    >
                    <v-icon
                      small
                      class="red--text text--lighten-2 mr-2"
                  ...
                  <td v-if="bug.fixed">
                    <v-icon
                      small
                      class="grey--text text--lighten-1 mr-2"
                      @click="unfixBug(bug._id)"
